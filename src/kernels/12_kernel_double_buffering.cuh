...
__global__ void __launch_bounds__(NUM_THREADS)
  ...
  // ── 为何不用 CUDA Stream，而用 cp.async + cuda::barrier ────────────────────
  // Stream 作用于设备级（inter-kernel）：重叠不同 kernel 或 cudaMemcpyAsync 的执行，
  // 无法控制同一 kernel 内部的执行顺序，也无法访问另一个 block 的 SMEM。
  //
  // cp.async（sm_80+ 专用指令）作用于 SM 级（intra-kernel）：
  //   在同一 block 内部，由 DMA 单元直接将 GMEM 数据写入 SMEM（绕过 L1 cache），
  //   不占用 CUDA Core 的 LD/ST 单元，因此可与 FPU 执行的 FMA 真正并行：
  //     [cp.async: tile N+1 → As[back]]  ← DMA 单元
  //     [FMA:      tile N   from As[front]] ← FPU（同时）
  //
  // cuda::barrier<thread_scope_block> 替代 __syncthreads()：
  //   arrive_and_wait() = 等待 block 内所有线程到达 + 等待关联的 cp.async 全部完成，
  //   粒度更细（可区分 front/back 两个 barrier），避免不必要的全 block 同步。
  // ────────────────────────────────────────────────────────────────────────────
